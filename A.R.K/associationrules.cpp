...
QWidget *AssociationRules::getTableRyles(){
        ...
        QStringList horizontalHeader;
          ...
          // verticalHeader.append("Ряд 2");
           ...
           //Первый ряд
           item = new QStandardItem(QString(list[i].split(",")[0]+"-->"+st));
           model->setItem(i-propuck, 0, item);

           item = new QStandardItem(QString(QString::number(sup,'f',2)));
           model->setItem(i-propuck, 1, item);

           item = new QStandardItem(QString(QString::number(cond,'f',2)));
           model->setItem(i-propuck, 2, item);

           item = new QStandardItem(QString(QString::number(improvement,'f',2)));
           model->setItem(i-propuck, 3, item);
  }
